...
import numpy as np
import pandas as pd
from attrs import converters, define, field, fields_dict, validators  # noqa: I900
...
def _valid_id(attribute: Attribute, value: Any) -> None:
...
class StateVar:
    ...
    ext_scale_add: float = field(
        kw_only=True, default=0, converter=converters.pipe(converters.default_if_none(0), float)
    )
    #: Value to multiply to the output from an external model (default: 1)
    ext_scale_mult: float = field(
        kw_only=True, default=1, converter=converters.pipe(converters.default_if_none(1), float)
    )
...
class StateConfig:
    ...
    def append_state(self, var: StateVar) -> None:
        ...
        if var.is_ext_input or var.is_ext_output:
            ...
            if var.ext_id is None:
                raise ValueError(
                    f"Variable {var.name} is specified as external input or output " f"but does not specify an ext_id."
                )
    ...
    def within_abort_conditions(self, state: Mapping[str, float]) -> bool:
        ...
        """
        valid = all(state[name] >= self.vars[name].abort_condition_min for name in self.abort_conditions_min)

        if valid:
            valid = all(state[name] <= self.vars[name].abort_condition_max for name in self.abort_conditions_max)
    ...
    def continuous_action_space(self) -> spaces.Box:
        ...
        :return: Action space
        """
        action_low = [var.low_value for var in self.vars.values() if var.is_agent_action and var.low_value is not None]
        action_high = [
            var.high_value for var in self.vars.values() if var.is_agent_action and var.high_value is not None
        ]

        return spaces.Box(action_low, action_high, dtype=np.float)
    ...
    def continuous_obs_space(self) -> spaces.Box:
        ...
        return spaces.Box(obs_low, obs_high, dtype=np.float)
