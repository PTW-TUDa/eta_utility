...
The module contains functions meant to simplify the general process of creating rolling horizons
optimization models and it contains the *ETAx* class which in turn combines all of this information
such that you can start simple optimizations in just two lines. For example, to start the pendulum
example (which is taken from the *gym* framework) with full configuration support, logging,
support for multiple series of optimization runs and many other things, execute the following script:
...
These algorithms specifially include some algorithms which are not from the field of reinforcement
learning but can be employed in generalized rolling horizons optimization settings.

Some base classes for environments, available in eta_utility.eta_x.envs make it
easy to create new environments in many cases (for example when using FMU simulation models) or when
performing model predictive control using mathematical models. These environments take away the work
of setting up models, passing around step results and so on.

*eta_x* extends the general Markov Decision Process by the option to introduce interactions between
multiple environments. This enables the creation of digital twins, which could for example use
a mathematical of simulation model for some aspects and interact with the actual devices for other
aspects.
...
---------------------------
*eta_x* build on the concept of experiments. An experiment can be configured to perform
...
As shown in the figure the process start with the configuration (setup) file, which is written in
JSON format (see :ref:`eta_experiment_config`). Based on this configuration and environment and
...
(such as being performed for a different time of the year).
...
The agent receives obervations from an environment and determines actions to control the environment
...
How to get started
--------------------
Usually you probably want to use the *ETAx* class as shown above to initialize your experiment.
...
minimal in that it makes extensive use of the defaults defined in the the *ConfigOpt* classes.
...
Below you can see the parameters that ConfigOptRum offers. Full documentation is in the API
